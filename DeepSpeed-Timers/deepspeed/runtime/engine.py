...
# , , , gate_time_whatever_part

        # gate timers =
        # gate_time_rsample_op, gate_time_gatesoftmax, gate_time_run_capacity, gate_time_createmasks, gate_time_maskused_tokens_einsum, gate_time_exp_counts_to_cpu, gate_time_no_drop_token_to_device
        # gate_time_compute_l_aux, gate_time_ds_random_token_sel, gate_time_top_idx_zeromask, gate_time_whatever_part, gate_time_cap_loc_norm, gate_time_one_hot_to_float, gate_time_einsum, gate_time_dispatch_mask
        top1_rsample_op = 0.0
        top1_softmax = 0.0
        top1_run_capacity = 0.0
        top1_createmasks = 0.0
        top1_maskused_tokens_einsum = 0.0
        top1_exp_counts_to_cpu= 0.0
        top1_no_drop_token_to_device = 0.0
        top1_compute_l_aux = 0.0
        top1_ds_random_token_sel= 0.0
        top1_time_top_idx_zeromask = 0.0
        top1_gate_time_whatever_part = 0.0
        top1_time_cap_loc_norm = 0.0
        top1_time_one_hot_to_float = 0.0
        top1_time_einsum = 0.0
        top1_dispatch_mask = 0.0
        # moe timers = reshape1, einsum, reshape2, expertcompute, reshape3, gather, einsum2, reshape4
        reshape1_time = 0.0
        einsum_time = 0.0
        reshape2_time = 0.0
        expertcompute_time = 0.0
        reshape3_time = 0.0
        gather_time = 0.0
        einsum2_time = 0.0
        reshape4_time = 0.0
        ...
        for gate in self.gate_modules:
            ...
            top1_rsample_op += gate.top1gate_timers['gate_time_rsample_op'] # 
            top1_softmax += gate.top1gate_timers['gate_time_gatesoftmax']
            top1_run_capacity+= gate.top1gate_timers['gate_time_run_capacity'] #
            top1_createmasks += gate.top1gate_timers['gate_time_createmasks']
            top1_maskused_tokens_einsum += gate.top1gate_timers['gate_time_maskused_tokens_einsum']
            top1_exp_counts_to_cpu += gate.top1gate_timers['gate_time_exp_counts_to_cpu']
            top1_no_drop_token_to_device += gate.top1gate_timers['gate_time_no_drop_token_to_device']
            top1_compute_l_aux += gate.top1gate_timers['gate_time_compute_l_aux']
            top1_ds_random_token_sel += gate.top1gate_timers['gate_time_ds_random_token_sel']
            top1_time_top_idx_zeromask += gate.top1gate_timers['gate_time_top_idx_zeromask']
            top1_gate_time_whatever_part += gate.top1gate_timers['gate_time_whatever_part']
            top1_time_cap_loc_norm += gate.top1gate_timers['gate_time_cap_loc_norm'] 
            top1_time_one_hot_to_float += gate.top1gate_timers['gate_time_one_hot_to_float']
            top1_time_einsum += gate.top1gate_timers['gate_time_einsum']
            top1_dispatch_mask += gate.top1gate_timers['gate_time_dispatch_mask']
        ...
        for l in self.moe_layers:
            ...
            # vasilis timers
            reshape1_time += l.time_reshape1
            einsum_time += l.time_einsum
            reshape2_time += l.time_reshape2
            expertcompute_time += l.time_expertcompute
            reshape3_time += l.time_reshape3
            gather_time += l.time_gather
            einsum2_time += l.time_einsum2
            reshape4_time += l.time_reshape4

        # TODO: Allreduce/average them across ranks for more accurate timing.

        # if deepspeed.comm.get_rank() == 0:
        log_dist(
            f"rank={dist.get_rank()} time (ms) | forward: {fwd_time:.2f} (forward_moe: {moe_time:.2f}, 1st alltoall: {falltoall:.2f}, 2nd alltoall: {salltoall:.2f}, top-k: {gate_time:.2f}, reshape1: {reshape1_time:.2f}, einsum: {einsum_time:.2f}, reshape2: {reshape2_time:.2f}, expertcompute: {expertcompute_time:.2f}, reshape3: {reshape3_time:.2f}, gather: {gather_time:.2f}, einsum2: {einsum2_time:.2f}, reshape4: {reshape4_time:.2f})",
            ranks=[0])
        log_dist(
            f"rank={dist.get_rank()} time (ms) | (topk-k total: {gate_time:.2f}, rsample_op: {top1_rsample_op:.2f}, softmax: {top1_softmax:.2f}, run_capacity: {top1_run_capacity:.2f}, create_masks: {top1_createmasks:.2f}, mask_unused_tokens: {top1_maskused_tokens_einsum:.2f}, exp_counts_to_cpu: {top1_exp_counts_to_cpu:.2f}, nodrop_to_device: {top1_no_drop_token_to_device:.2f}, compute_l_aux: {top1_compute_l_aux:.2f}, ds_random_token_sel: {top1_ds_random_token_sel:.2f}, top_idx_zeromask: {top1_time_top_idx_zeromask:.2f}, whatever_part: {top1_gate_time_whatever_part:.2f}, capacity_loc_norm: {top1_time_cap_loc_norm:.2f}, onehot2float: {top1_time_one_hot_to_float:.2f}, einsum: {top1_time_einsum:.2f}, dispatch_mask: {top1_dispatch_mask:.2f})",
            ranks=[0])
